...
object DTS {
  ...
  private case class Cells(parentAddress: Int, parentSize: Int, selfAddress: Int, selfSize: Int)
  ...
  private def fmtMapping(x: ResourceMapping, cells: Cells): Seq[String] = {
    ...
    ranges.flatMap {
      case AddressRange(base, size) =>
        fmtCell(base + x.offset, cells.selfAddress) ++ fmtCell(base, cells.parentAddress) ++ fmtCell(
          size,
          cells.selfSize
        )
    }
