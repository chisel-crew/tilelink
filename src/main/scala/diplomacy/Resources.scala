 ...
 * @param permissions  the permission attributes of this space. See [[tetralink.diplomacy.ResourcePermissions]].
 */
 ...
 * @param name         the resolved name of this device. See [[tetralink.diplomacy.DeviceRegName]].
 ...
 */
class SimpleDevice(val devname: String, devcompat: Seq[String])
    extends Device
...
trait BindingScope {
  ...
  protected[diplomacy] var resourceBindingFns
    : Seq[() => Unit]                                                                       = Nil // callback functions to resolve resource binding during elaboration
  ...
  private def collect(
    ...
    def mapChildren(offset: BigInt) = map.value.toList.flatMap {
      case (key, seq) =>
        seq.collect {
  ...
  def bindingTree: ResourceMap = {
    ...
    val tree = makeTree(descs.toList.flatMap {
      case (d, Description(name, mapping)) =>
  ...
  def getResourceBindingsMap: ResourceBindingsMap = {
    eval
    ResourceBindingsMap(map =
      resourceBindings.reverse
        .groupBy(_._1.owner)
        .mapValues(seq => ResourceBindings(seq.groupBy(_._1.key).mapValues(_.map(z => Binding(z._2, z._3)).distinct)))
    )
  }
