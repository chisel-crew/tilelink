...
import tetralink.amba.axi4._
import tetralink.amba._
...
class TLtoAXI4IdMap(tl: TLMasterPortParameters, axi4: AXI4MasterPortParameters) {
  ...
  val mapping: Seq[TLToAXI4IdMapEntry] = (sorted zip axi4.masters) map {
    case (c, m) =>
...
case class TLToAXI4Node(stripBits: Int = 0, wcorrupt: Boolean = true)(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, AXI4Imp)(dFn = { p =>
      p.clients.foreach { c =>
        require(
          c.sourceId.start % (1 << stripBits) == 0 &&
      ...
      val masters = ((idStart zip idSize) zip clients) map {
        case ((start, size), c) =>
      ...
      AXI4MasterPortParameters(
        masters = masters,
        requestFields =
          (if (wcorrupt) Seq(AMBACorruptField()) else Seq()) ++ p.requestFields.filter(!_.isInstanceOf[AMBAProtField]),
        echoFields = AXI4TLStateField(log2Ceil(p.endSourceId)) +: p.echoFields,
        responseKeys = p.responseKeys
      )
    }, uFn = { p =>
      TLSlavePortParameters.v1(
        managers = p.slaves.map {
          case s =>
...
class TLToAXI4(
  val combinational: Boolean = true,
  val adapterName: Option[String] = None,
  val stripBits: Int = 0,
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        Annotated.idMapping(this, map.mapping).foreach {
          case TLToAXI4IdMapEntry(axi4Id, tlId, _, _, fifo) =>
        ...
        (a_sel zip d_sel zip idStall zip idCount) foreach {
          case (((as, ds), s), n) =>
            ...
            val mismatch = if (maxCount > 1) { write =/= arw.wen }
            else { Bool(false) }
