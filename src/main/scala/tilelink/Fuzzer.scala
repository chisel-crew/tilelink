 ...
 */
class TLFuzzer(nOperations: Int, inFlight: Int = 32, noiseMaker: (Int, Bool, Int) => UInt = {
  (wide: Int, increment: Bool, abs_values: Int) => LFSRNoiseMaker(wide = wide, increment = increment)
}, noModify: Boolean = false, overrideAddress: Option[AddressSet] = None, nOrdered: Option[Int] = None)(
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val legal = legal_dest && MuxLookup(
      a_type_sel,
      glegal,
      Seq(
        UInt("b000") -> glegal,
...
object TLFuzzer {
  def apply(nOperations: Int, inFlight: Int = 32, noiseMaker: (Int, Bool, Int) => UInt = {
    (wide: Int, increment: Bool, abs_values: Int) => LFSRNoiseMaker(wide = wide, increment = increment)
  }, noModify: Boolean = false, overrideAddress: Option[AddressSet] = None, nOrdered: Option[Int] = None)(
    implicit p: Parameters
  ): TLOutwardNode = {
