...
object ForceFanout {
  def apply[T](
    ...
    e: TriStateValue = TriStateValue.unset
  )(body: Parameters => T)(implicit p: Parameters) =
    body(p.alterPartial {
      case ForceFanoutKey =>
        p(ForceFanoutKey) match {
...
class TLXbar(policy: TLArbiter.Policy = TLArbiter.roundRobin)(implicit p: Parameters) extends LazyModule {
      ...
      seq(0).v1copy(
        ...
        managers = seq.flatMap { port =>
          require(
            ...
            s"Xbar ($name with parent $parent) data widths don't match: ${port.managers
              .map(_.name)} has ${port.beatBytes}B vs ${seq(0).managers.map(_.name)} has ${seq(0).beatBytes}B"
          )
  ...
  lazy val module = new LazyModuleImp(this) {
    if ((node.in.size * node.out.size) > (8 * 32)) {
      println(s"!!! WARNING !!!")
      println(
        s" Your TLXbar ($name with parent $parent) is very large, with ${node.in.size} Masters and ${node.out.size} Slaves."
      )
      println(s"!!! WARNING !!!")
    }
...
object TLXbar {
  def circuit(policy: TLArbiter.Policy, seqIn: Seq[(TLBundle, TLEdge)], seqOut: Seq[(TLBundle, TLEdge)]) {
    ...
    val reachableIO = edgesIn.map { cp =>
      edgesOut.map { mp =>
        cp.client.clients.exists { c =>
          mp.manager.managers.exists(m => c.visibility.exists(ca => m.address.exists(ma => ca.overlaps(ma))))
        }
      }.toVector
    }.toVector
    val probeIO = (edgesIn zip reachableIO).map {
      case (cp, reachableO) =>
    ...
    val releaseIO = (edgesIn zip reachableIO).map {
      case (cp, reachableO) =>
        (edgesOut zip reachableO).map {
          case (mp, reachable) =>
    ...
    val portsAOI = transpose((in zip requestAIO) map {
      case (i, r) => TLXbar.fanout(i.a, r, edgesOut.map(_.params(ForceFanoutKey).a))
    })
    val portsBIO = transpose((out zip requestBOI) map {
      case (o, r) => TLXbar.fanout(o.b, r, edgesIn.map(_.params(ForceFanoutKey).b))
    })
    val portsCOI = transpose((in zip requestCIO) map {
      case (i, r) => TLXbar.fanout(i.c, r, edgesOut.map(_.params(ForceFanoutKey).c))
    })
    val portsDIO = transpose((out zip requestDOI) map {
      case (o, r) => TLXbar.fanout(o.d, r, edgesIn.map(_.params(ForceFanoutKey).d))
    })
    val portsEOI = transpose((in zip requestEIO) map {
      case (i, r) => TLXbar.fanout(i.e, r, edgesOut.map(_.params(ForceFanoutKey).e))
    })
  ...
  def fanout[T <: TLChannel](
    input: DecoupledIO[T],
    select: Seq[Bool],
    force: Seq[Boolean] = Nil
  ): Seq[DecoupledIO[T]] = {
...
class TLRAMXbarTest(nManagers: Int, txns: Int = 5000, timeout: Int = 500000)(implicit p: Parameters)
...
class TLMulticlientXbarTest(nManagers: Int, nClients: Int, txns: Int = 5000, timeout: Int = 500000)(
