...
import tetralink.devices.tilelink._
import tetralink.prci._
import tetralink.subsystem._
...
abstract class TLBusWrapper(params: HasTLBusParams, val busName: String)(implicit p: Parameters)
  ...
  private val clockGroupAggregator =
    LazyModule(new ClockGroupAggregator(busName)).suggestName(busName + "_clock_groups")
  ...
  def crossToBus(bus: TLBusWrapper, xType: ClockCrossingType)(
    implicit asyncClockGroupNode: ClockGroupEphemeralNode
  ): NoHandle = {
  ...
  def crossFromBus(bus: TLBusWrapper, xType: ClockCrossingType)(
    implicit asyncClockGroupNode: ClockGroupEphemeralNode
  ): NoHandle = {
    bus.clockGroupNode := asyncMux(xType, asyncClockGroupNode, this.clockGroupNode)
...
trait TLBusWrapperConnectionLike {
  ...
  def connect(context: HasTileLinkLocations, master: Location[TLBusWrapper], slave: Location[TLBusWrapper])(
...
object TLBusWrapperConnection {
  ...
  def crossTo(
    ...
    apply(xType, driveClockFromMaster, nodeBinding, flipRendering)(slaveNodeView = {
      case (w, p) => w.crossInHelper(xType)(p)
    })
  ...
  def crossFrom(
    ...
    apply(xType, driveClockFromMaster, nodeBinding, flipRendering)(masterNodeView = {
      case (w, p) => w.crossOutHelper(xType)(p)
    })
  ...
  def apply(
    ...
    flipRendering: Boolean = false
  )(slaveNodeView: (TLBusWrapper, Parameters) => TLInwardNode = { case (w, _) => w.inwardNode }, masterNodeView: (TLBusWrapper, Parameters) => TLOutwardNode = {
    case (w, _)                                                               => w.outwardNode
  }, inject: Parameters => TLNode = { _ => TLTempNode() }) =
    new TLBusWrapperConnection(xType, driveClockFromMaster, nodeBinding, flipRendering)(
      slaveNodeView,
      masterNodeView,
      inject
    )
...
class TLBusWrapperConnection(
  ...
  def connect(context: HasTileLinkLocations, master: Location[TLBusWrapper], slave: Location[TLBusWrapper])(
    ...
    def bindTLNodes(implicit p: Parameters) = nodeBinding match {
      case BIND_ONCE =>
        slaveNodeView(slaveTLBus, p) := TLWidthWidget(masterTLBus.beatBytes) := inject(p) := masterNodeView(
          masterTLBus,
          p
        )
      case BIND_QUERY =>
...
trait CanAttachTLSlaves extends HasTLBusParams { this: TLBusWrapper =>
  ...
  def toTile(name: Option[String] = None, buffer: BufferParams = BufferParams.none)(gen: => TLInwardNode): NoHandle =
    to("tile" named name) {
      FlipRendering(implicit p => gen :*= TLWidthWidget(beatBytes) :*= TLBuffer(buffer) :*= outwardNode)
    }

  def toDRAMController[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[TLClientPortParameters, TLManagerPortParameters, TLEdgeIn, TLBundle, D, U, E, B] = TLNameNode(
      name
    )
  ): OutwardNodeHandle[D, U, E, B] =
  ...
  def toVariableWidthSlaveNode(name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    node: TLInwardNode
  ) {
    toVariableWidthSlaveNodeOption(name, buffer)(Some(node))
  }

  def toVariableWidthSlaveNodeOption(name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    node: Option[TLInwardNode]
  ) {
    node foreach { n =>
      to("slave" named name) {
  ...
  def toVariableWidthSlave[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[TLClientPortParameters, TLManagerPortParameters, TLEdgeIn, TLBundle, D, U, E, B] = TLNameNode(
      name
    )
  ): OutwardNodeHandle[D, U, E, B] =
  ...
  def toFixedWidthSlave[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[TLClientPortParameters, TLManagerPortParameters, TLEdgeIn, TLBundle, D, U, E, B] = TLNameNode(
      name
    )
  ): OutwardNodeHandle[D, U, E, B] =
  ...
  def toFixedWidthPort[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[TLClientPortParameters, TLManagerPortParameters, TLEdgeIn, TLBundle, D, U, E, B] = TLNameNode(
      name
    )
  ): OutwardNodeHandle[D, U, E, B] =
...
trait CanAttachTLMasters extends HasTLBusParams { this: TLBusWrapper =>
  def fromTile(name: Option[String], buffer: BufferParams = BufferParams.none, cork: Option[Boolean] = None)(
    gen: => TLOutwardNode
  ): NoHandle =
  ...
  def fromMasterNode(name: Option[String] = None, buffer: BufferParams = BufferParams.none)(gen: TLOutwardNode) {
    from("master" named name) {
      inwardNode :=* TLBuffer(buffer) :=* TLFIFOFixer(TLFIFOFixer.all) :=* gen
    }
  }

  def fromMaster[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[D, U, E, B, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLBundle] = TLNameNode(
      name
    )
  ): InwardNodeHandle[D, U, E, B] =
    from("master" named name) {
      inwardNode :=* TLBuffer(buffer) :=* TLFIFOFixer(TLFIFOFixer.all) :=* gen
    }

  def fromPort[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[D, U, E, B, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLBundle] = TLNameNode(
      name
    )
  ): InwardNodeHandle[D, U, E, B] =
  ...
  def fromCoherentMaster[D, U, E, B <: Data](name: Option[String] = None, buffer: BufferParams = BufferParams.none)(
    gen: => NodeHandle[D, U, E, B, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLBundle] = TLNameNode(
      name
    )
  ): InwardNodeHandle[D, U, E, B] =
...
class TLJBarWrapper(params: TLJBarWrapperParams, name: String)(implicit p: Parameters)
    extends TLBusWrapper(params, name) {
