...
class TLRAMModel(log: String = "", ignoreCorruptData: Boolean = false, ignoreDeniedData: Boolean = true)(
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        val bypass =
          if (edge.manager.minLatency > 0) Bool(false) else in.a.valid && in.a.bits.source === out.d.bits.source
        ...
        when(a_fire) {
          ...
          when(
            a.opcode === TLMessages.PutFullData || a.opcode === TLMessages.PutPartialData ||
        ...
        when(d_fire) {
          ...
          when(
            d_flight.opcode === TLMessages.Get || d_flight.opcode === TLMessages.ArithmeticData || d_flight.opcode === TLMessages.LogicalData
          ) {
...
object TLRAMModel {
  def apply(log: String = "", ignoreCorruptData: Boolean = false, ignoreDeniedData: Boolean = true)(
    implicit p: Parameters
  ): TLNode = {
