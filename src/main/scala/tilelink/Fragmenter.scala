...
class TLFragmenter(
  val minSize: Int,
  val maxSize: Int,
  val alwaysMin: Boolean = false,
  val earlyAck: EarlyAck.T = EarlyAck.None,
  val holdFirstDeny: Boolean = false
  ...
  def expandTransfer(x: TransferSizes, op: String) =
    if (!x) x
    else {
      // validate that we can apply the fragmenter correctly
      require(
        x.max >= minSize,
        s"TLFragmenter (with parent $parent) max transfer size $op(${x.max}) must be >= min transfer size (${minSize})"
      )
  ...
  def shrinkTransfer(x: TransferSizes) =
    if (!alwaysMin) x
      ...
      TransferSizes.none
  def mapManager(m: TLSlaveParameters) =
    m.v1copy(
    ...
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        if (minSize == maxSize) {
          ...
          require(
            minSize >= beatBytes,
            s"TLFragmenter (with parent $parent) can't support fragmenting ($minSize) to sub-beat ($beatBytes) accesses"
          )
          ...
          require(
            !edgeOut.manager.mayDenyGet || holdFirstDeny,
            s"TLFragmenter (with parent $parent) can't support denials without holdFirstDeny=true"
          )
          ...
          out.a <> in_a
          out.a.bits.address := in_a.bits.address | ~(old_gennum1 << log2Ceil(beatBytes) | ~aOrigOH1 | aFragOH1 | UInt(
            minSize - 1
          ))
...
object TLFragmenter {
  def apply(
    minSize: Int,
    maxSize: Int,
    alwaysMin: Boolean = false,
    earlyAck: EarlyAck.T = EarlyAck.None,
    holdFirstDeny: Boolean = false
...
class TLRAMFragmenterTest(ramBeatBytes: Int, maxSize: Int, txns: Int = 5000, timeout: Int = 500000)(
  implicit p: Parameters
