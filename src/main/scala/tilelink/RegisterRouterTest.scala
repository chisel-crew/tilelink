...
import tetralink.regmapper._
import tetralink.unittest._
import tetralink.util.{ Pow2ClockDivider }
...
class RRTestRequest(
  ...
  wflow: (Bool, Bool, UInt) => (Bool, Bool, UInt)
) extends Module {
  val io = new Bundle {
...
object RRTestRequest {
  ...
  def pipe(x: Int): (Bool, Bool, UInt) => (Bool, Bool, UInt) = { (ivalid, oready, idata) =>
    ...
    ((ready.init zip ready.tail) zip full.init) foreach {
      case ((self, next), full) =>
        self := next || !full
    }
    ((data.init zip data.tail) zip ready.tail) foreach {
      case ((prev, self), ready) =>
        when(ready)(self := prev)
    }
    ((full.init zip full.tail) zip ready.tail) foreach {
      case ((prev, self), ready) =>
        when(ready)(self := prev)
    }
...
class RRTest0(address: BigInt)(implicit p: Parameters)
    extends TLRegisterRouter(address, "test0", Nil, 0, 32, 0, 4)(new TLRegBundle((), _) with RRTest0Bundle)(
...
class RRTest1(address: BigInt)(implicit p: Parameters)
    extends TLRegisterRouter(address, "test1", Nil, 0, 32, 6, 4)(new TLRegBundle((), _) with RRTest1Bundle)(
