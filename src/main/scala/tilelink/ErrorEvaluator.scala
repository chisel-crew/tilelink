...
class TLErrorEvaluator(test: RequestPattern, testOn: Boolean, testOff: Boolean, deny: Boolean = false)(
  implicit p: Parameters
) extends LazyModule {
  val node = TLAdapterNode(managerFn = { mp =>
    mp.v1copy(managers = mp.managers.map(m => m.v1copy(mayDenyPut = true, mayDenyGet = deny || m.mayDenyGet)))
  })
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        assert(
          Bool(!testOn) || !out.d.fire() || !d_last || !d_inject || d_detect || d_hint,
          "Denied/Corrupt flag was not set!"
        )
...
object TLErrorEvaluator {
  def apply(test: RequestPattern, testOn: Boolean = false, testOff: Boolean = false, deny: Boolean = false)(
    implicit p: Parameters
  ): TLNode = {
