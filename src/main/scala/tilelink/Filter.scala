...
class TLFilter(
  ...
  val node = TLAdapterNode(
    clientFn = { cp =>
      cp.v1copy(clients = cp.clients.flatMap {
        c =>
          val out = cfilter(c)
          out.map {
            o => // Confirm the filter only REMOVES capability
    ...
    managerFn = { mp =>
      val managers = mp.managers.flatMap {
        m =>
          val out = mfilter(m)
          out.map {
            o => // Confirm the filter only REMOVES capability
      ...
      mp.v1copy(managers = managers, endSinkId = if (managers.exists(_.supportsAcquireB)) mp.endSinkId else 0)
    }
  )
...
object TLFilter {
  ...
  def mHideCacheable: ManagerFilter = { m => if (m.supportsAcquireB) None else Some(m) }
  // make visible only cacheable managers
  def mSelectCacheable: ManagerFilter = { m => if (m.supportsAcquireB) Some(m) else None }
  ...
  def cHideCaching: ClientFilter = { c => if (c.supportsProbe) None else Some(c) }
  // onyl caching clients are visible
  def cSelectCaching: ClientFilter = { c => if (c.supportsProbe) Some(c) else None }
