...
import tetralink.diplomaticobjectmodel.logicaltree.{ BusMemoryLogicalTreeNode, LogicalModuleTree, LogicalTreeNode }
import tetralink.diplomaticobjectmodel.model.{ OMECC, TL_UL }
import tetralink.util._
import tetralink.util.property._
...
class TLRAM(
  ...
  val dtsCompat: Option[Seq[String]] = None
    ...
    extends DiplomaticSRAM(address, beatBytes, devName, dtsCompat) {
  ...
  require(
    eccBytes <= beatBytes,
    s"TLRAM eccBytes (${eccBytes}) > beatBytes (${beatBytes}). Use a WidthWidget=>Fragmenter=>SRAM if you need high density and narrow ECC; it will do bursts efficiently"
  )
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val (d_wb_lanes, d_wb_poison) = Seq
      .tabulate(lanes) { i =>
        val upd = d_rmw_mask(eccBytes * (i + 1) - 1, eccBytes * i)
        (
          upd.orR || d_correctable(i),
          (!upd.andR && d_uncorrectable(i)) || d_poison
        ) // sub-lane writes should not correct uncorrectable
      }
      .unzip
...
//  val ren = !d_wb && (a_fire &&  a_ren)
    ...
    val coded = Vec((dat zip poison) map {
      case (d, p) =>
...
object TLRAM {
  def apply(
    ...
    val ram = LazyModule(
      new TLRAM(address, parentLogicalTreeNode, cacheable, executable, atomics, beatBytes, ecc, devName)
    )
    ram.node
  }
}
...
class TLRAMSimpleTest(ramBeatBytes: Int, txns: Int = 5000, timeout: Int = 500000)(implicit p: Parameters)
...
class TLRAMECCTest(ramBeatBytes: Int, eccBytes: Int, txns: Int = 5000, timeout: Int = 500000)(implicit p: Parameters)
    extends UnitTest(timeout) {
