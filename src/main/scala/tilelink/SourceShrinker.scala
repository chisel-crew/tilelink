...
class TLSourceShrinker(maxInFlight: Int)(implicit p: Parameters) extends LazyModule {
  ...
  private val client = TLMasterParameters.v1(name = "TLSourceShrinker", sourceId = IdRange(0, maxInFlight))
  val node = TLAdapterNode(
    ...
    managerFn = { mp =>
      mp.v1copy(managers = mp.managers.map(m => m.v1copy(fifoId = if (maxInFlight == 1) Some(0) else m.fifoId)))
    }
  )
    ...
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        if (noShrinkRequired(edgeIn.client)) {
          ...
          val bypass =
            Bool(edgeOut.manager.minLatency == 0) && in.a.valid && !full && a_first && nextFree === out.d.bits.source
          in.d <> out.d
