        ...
        val managers = sp.slaves.map {
          case s =>
            TLSlaveParameters.v1(
              ...
              supportsHint =
                if (!supportHints) TransferSizes.none
                else if (s.supportsRead) s.supportsRead
                else if (s.supportsWrite) s.supportsWrite
                else
...
class TLToAHB(val aFlow: Boolean = false, val supportHints: Boolean = true, val supportsRETRY: Boolean = true)(
      ...
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        when(send.send && !a_flow) /* retry AHB */ {
          ...
          .elsewhen(send.full && !send.last) /* continue burst */ {
            ...
            step.last := (if (lgBytes + 1 >= lgMax) Bool(true)
                          else
...
object TLToAHB {
  def apply(aFlow: Boolean = true, supportHints: Boolean = true, supportsRETRY: Boolean = true)(
    implicit p: Parameters
  ) = {
