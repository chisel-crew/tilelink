...
case class TLMasterToSlaveTransferSizes(
  ...
  def intersect(rhs: TLMasterToSlaveTransferSizes) =
    TLMasterToSlaveTransferSizes(
  ...
  def cover(rhs: TLMasterToSlaveTransferSizes) =
    TLMasterToSlaveTransferSizes(
...
class TLSlaveParameters private (
  ...
  val mayDenyPut: Boolean
) // applies to: AccessAck,     Grant,    HintAck
...
object TLChannelBeatBytes {
  def apply(beatBytes: Int): TLChannelBeatBytes =
    TLChannelBeatBytes(Some(beatBytes), Some(beatBytes), Some(beatBytes), Some(beatBytes))
  def apply(): TLChannelBeatBytes = TLChannelBeatBytes(None, None, None, None)
}
...
class TLSlavePortParameters private (
  ...
  def beatBytes = {
    channelBytes.members.foreach(width => require(width.isDefined && width == channelBytes.a))
    channelBytes.a.get
  }
  ...
  def fastPropertyGroup[K](p: TLManagerParameters => K): Seq[(K, Seq[AddressSet])] = {
    val groups = groupByIntoSeq(managers.map(m => (p(m), m.address)))(_._1).map {
      case (k, vs) =>
        k -> vs.flatMap(_._2)
    }
  ...
  private def supportHelper(
    ...
    val supportCases = groupByIntoSeq(managers)(m => trim(member(m))).map {
      case (k, vs) =>
  ...
  def copy(
    ...
    v1copy(managers, beatBytes, endSinkId, minLatency, responseFields, requestKeys)
}
...
object TLManagerPortParameters {
  ...
  def apply(
    ...
    TLSlavePortParameters.v1(managers, beatBytes, endSinkId, minLatency, responseFields, requestKeys)
}
...
class TLMasterPortParameters private (
  ...
  IdRange.overlaps(clients.map(_.sourceId)).foreach {
    case (x, y) =>
  ...
  def unusedSources: Seq[Int] = {
    ...
    ((Seq(0) ++ usedSources.map(_.end)) zip usedSources.map(_.start)) flatMap {
      case (end, start) =>
        end until start
    }
  }
  ...
  private def safety_helper(member: TLClientParameters => TransferSizes)(id: UInt, lgSize: UInt) = {
    ...
    if (allSame) member(clients(0)).containsLg(lgSize)
    else {
  ...
  def copy(
    ...
    v1copy(clients, minLatency, echoFields, requestFields, responseKeys)
}
...
object TLClientPortParameters {
  ...
  def apply(
    ...
    TLMasterPortParameters.v1(clients, minLatency, echoFields, requestFields, responseKeys)
}
...
case class TLAsyncManagerPortParameters(async: AsyncQueueParams, base: TLManagerPortParameters) {
  def infoString = base.infoString
}
...
case class TLRationalEdgeParameters(
  client: TLRationalClientPortParameters,
  manager: TLRationalManagerPortParameters,
  params: Parameters,
  sourceInfo: SourceInfo
) extends FormatEdge {
...
object ManagerUnificationKey {
  def apply(x: TLManagerParameters): ManagerUnificationKey =
    ManagerUnificationKey(
...
object ManagerUnification {
  def apply(managers: Seq[TLManagerParameters]): List[TLManagerParameters] =
    managers
      .groupBy(ManagerUnificationKey.apply)
      .values
      .map { seq =>
        val agree = seq.forall(_.fifoId == seq.head.fifoId)
        seq(0).v1copy(address = AddressSet.unify(seq.flatMap(_.address)), fifoId = if (agree) seq(0).fifoId else None)
      }
      .toList
}
...
class TLSourceIdMap(tl: TLClientPortParameters) {
  ...
  val mapping: Seq[TLSourceIdMapEntry] = sorted.map {
    case c =>
