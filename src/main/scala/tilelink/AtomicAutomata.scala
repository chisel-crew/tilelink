...
class TLAtomicAutomata(
  ...
  val node = TLAdapterNode(managerFn = {
    case mp =>
      mp.v1copy(managers = mp.managers.map {
        m =>
          val ourSupport = TransferSizes(1, mp.beatBytes)
          def widen(x: TransferSizes) =
            if (passthrough && x.min <= 2 * mp.beatBytes) TransferSizes(1, max(mp.beatBytes, x.max)) else ourSupport
          ...
          require(
            !m.supportsPutFull || !m.supportsGet || canDoit,
            s"${m.name} has $ourSupport, needed PutFull(${m.supportsPutFull}) or Get(${m.supportsGet})"
          )
          m.v1copy(
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        val violations =
          managersNeedingHelp.flatMap(_.findTreeViolation).map(node => (node.name, node.inputs.map(_._1.name)))
        require(
          violations.isEmpty,
        ...
        if (camSize > 0) {
          ...
          // AMO result data
          val amo_data =
            if (!logical) arith_out
            else if (!arithmetic) logic_out
            else
          ...
          when(source_i.fire() && !a_isSupported) {
            (a_cam_sel_free zip cam_a) foreach {
              case (en, r) =>
                when(en) {
                  r.fifoId := a_fifoId
                  r.bits := in.a.bits
            ...
            (a_cam_sel_free zip cam_s) foreach {
              case (en, r) =>
                when(en) {
                  r.state := GET
                }
          ...
          when(source_c.fire()) {
            (a_cam_sel_put zip cam_s) foreach {
              case (en, r) =>
                when(en) {
                  r.state := ACK
                }
          ...
          val d_cam_sel_bypass =
            if (edgeOut.manager.minLatency > 0) Bool(false)
            else
...
object TLAtomicAutomata {
  def apply(logical: Boolean = true, arithmetic: Boolean = true, concurrency: Int = 1, passthrough: Boolean = true)(
    implicit p: Parameters
  ): TLNode = {
...
class TLRAMAtomicAutomataTest(txns: Int = 5000, timeout: Int = 500000)(implicit p: Parameters)
