...
import tetralink.diplomaticobjectmodel.model.{ OMMemoryRegion, OMRegister, OMRegisterMap }
import tetralink.regmapper._
import tetralink.interrupts._
...
case class TLRegisterNode(
  ...
  executable: Boolean = false
    ...
    with TLFormatNode // the Queue adds at most one cycle
    {
  ...
  def genRegDescsJson(mapping: RegField.Map*) {
    ...
    GenRegDescsAnno.anno(module, base, mapping: _*)
...
)(bundleBuilder: TLRegBundleArg => B)(moduleBuilder: (=> B, TLRegisterRouterBase) => M)(implicit p: Parameters)
    extends TLRegisterRouterBase(
      devname,
      devcompat,
      AddressSet(base, size - 1),
      interrupts,
      concurrency,
      beatBytes,
      undefZero,
