...
case class BankBinderNode(mask: BigInt)(implicit valName: ValName) extends TLCustomNode {
  ...
  def resolveStar(iKnown: Int, oKnown: Int, iStars: Int, oStars: Int): (Int, Int) = {
    ...
    require(
      ports == iKnown + iStar * iStars,
      s"${name} must have ${ports} inputs, but has ${iKnown} + ${iStar}*${iStars} (at ${lazyModule.line})"
    )
    require(
      ports == oKnown + oStar * oStars,
      s"${name} must have ${ports} outputs, but has ${oKnown} + ${oStar}*${oStars} (at ${lazyModule.line})"
    )
    (iStar, oStar)
  }
  ...
  def mapParamsU(n: Int, p: Seq[TLSlavePortParameters]): Seq[TLSlavePortParameters] =
    (p zip ids) map {
      case (mp, id) =>
        mp.v1copy(managers = mp.managers.flatMap { m =>
