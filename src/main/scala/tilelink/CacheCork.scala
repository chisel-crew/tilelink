    ...
    clientFn = {
      case cp =>
        cp.v1copy(clients = cp.clients.map { c =>
          c.v1copy(supportsProbe = TransferSizes.none, sourceId = IdRange(c.sourceId.start * 2, c.sourceId.end * 2))
        })
    },
    managerFn = {
      case mp =>
    ...
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        if (!edgeIn.manager.anySupportAcquireB) {
          ...
          require(
            clients.size == 1 || caches.size == 0 || unsafe,
            s"Only one client can safely use a TLCacheCork; ${clients.map(_.name)}"
          )
          require(caches.size <= 1 || unsafe, s"Only one caching client allowed; ${clients.map(_.name)}")
          edgeOut.manager.managers.foreach {
            case m =>
          ...
          edgeIn.client.unusedSources.foreach(id => wSourceVec(id) := Bool(edgeIn.manager.anySupportPutFull))
          ...
          TLArbiter(TLArbiter.lowestIndexFirst)(
            out.a,
            (edgeOut.numBeats1(c_a.bits), c_a),
            (edgeOut.numBeats1(a_a.bits), a_a)
          )
          TLArbiter(TLArbiter.lowestIndexFirst)(
            in_d,
            (edgeIn.numBeats1(d_d.bits), d_d),
            (UInt(0), Queue(c_d, 2)),
            (UInt(0), Queue(a_d, 2))
          )
