...
import tetralink.util.CompileOptions.NotStrictInferReset
import tetralink.config.Parameters
import tetralink.diplomacy._
import tetralink.util._
import tetralink.amba.AMBAProt
...
class TLBroadcast(lineBytes: Int, numTrackers: Int = 4, bufferless: Boolean = false)(implicit p: Parameters)
  ...
  val node = TLAdapterNode(
    clientFn = { cp =>
      cp.v1copy(clients =
        Seq(TLMasterParameters.v1(name = "TLBroadcast", sourceId = IdRange(0, 1 << log2Ceil(cp.endSourceId * 4))))
      )
    },
    managerFn = { mp =>
      mp.v1copy(
        endSinkId = numTrackers,
        managers = mp.managers.map {
          m =>
            ...
            if (m.regionType == RegionType.UNCACHED) {
              ...
              require(
                ...
                s"${m.name} only supports PutFull(${m.supportsPutFull}), which does not include $lineBytes"
              )
              require(
                !m.supportsGet || m.supportsGet.contains(lineBytes),
                s"${m.name} only supports Get(${m.supportsGet}), which does not include $lineBytes"
              )
              m.v1copy(
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        (trackers zip UIntToOH(in.e.bits.sink).asBools) foreach {
          case (tracker, select) =>
        ...
        (trackers zip d_trackerOH.asBools) foreach {
          case (tracker, select) =>
        ...
        (trackers zip c_trackerOH) foreach {
          case (tracker, select) =>
        ...
        (trackers zip c_trackerOH) foreach {
          case (tracker, select) =>
            tracker.probesack := in.c
              .fire() && select && (c_probeack || c_probeackdata) && (in.c.bits.param === TLPermissions.TtoB ||
        ...
        (trackers zip selectTracker.asBools) foreach {
          case (t, select) =>
        ...
        when(in.a.fire() && a_first) {
          ...
          probe_perms := MuxLookup(
            ...
            Array(
              ...
              TLMessages.Hint -> MuxLookup(
                in.a.bits.param,
                Wire(UInt(width = 2)),
                Array(TLHints.PREFETCH_READ -> TLPermissions.toB, TLHints.PREFETCH_WRITE -> TLPermissions.toN)
              ),
