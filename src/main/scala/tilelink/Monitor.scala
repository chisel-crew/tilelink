...
import tetralink.util.{ HeterogeneousBag, PlusArg }
import tetralink.formal._
...
@chiselName
class TLMonitor(args: TLMonitorArgs, monitorDir: MonitorDirection = MonitorDirection.Monitor)
    extends TLMonitorBase(args) {
  ...
  def legalizeFormatA(bundle: TLBundleA, edge: TLEdge) {
    ...
    monAssert(
      visible(edge.address(bundle), bundle.source, edge),
      "'A' channel carries an address illegal for the specified bank visibility"
    )

    when(bundle.opcode === TLMessages.AcquireBlock) {
      monAssert(
        edge.manager.supportsAcquireBSafe(edge.address(bundle), bundle.size),
        "'A' channel carries AcquireBlock type unsupported by manager" + extra
      )
      monAssert(
        edge.client.supportsProbe(edge.source(bundle), bundle.size),
        "'A' channel carries AcquireBlock from a client which does not support Probe" + extra
      )
      monAssert(source_ok, "'A' channel AcquireBlock carries invalid source ID" + extra)
      monAssert(
        bundle.size >= log2Ceil(edge.manager.beatBytes).U,
        "'A' channel AcquireBlock smaller than a beat" + extra
      )
    ...
    when(bundle.opcode === TLMessages.AcquirePerm) {
      monAssert(
        edge.manager.supportsAcquireBSafe(edge.address(bundle), bundle.size),
        "'A' channel carries AcquirePerm type unsupported by manager" + extra
      )
      monAssert(
        edge.client.supportsProbe(edge.source(bundle), bundle.size),
        "'A' channel carries AcquirePerm from a client which does not support Probe" + extra
      )
      monAssert(source_ok, "'A' channel AcquirePerm carries invalid source ID" + extra)
      monAssert(
        bundle.size >= log2Ceil(edge.manager.beatBytes).U,
        "'A' channel AcquirePerm smaller than a beat" + extra
      )
    ...
    when(bundle.opcode === TLMessages.Get) {
      monAssert(
        edge.manager.supportsGetSafe(edge.address(bundle), bundle.size),
        "'A' channel carries Get type unsupported by manager" + extra
      )
    ...
    when(bundle.opcode === TLMessages.PutFullData) {
      monAssert(
        edge.manager.supportsPutFullSafe(edge.address(bundle), bundle.size),
        "'A' channel carries PutFull type unsupported by manager" + extra
      )
    ...
    when(bundle.opcode === TLMessages.PutPartialData) {
      monAssert(
        edge.manager.supportsPutPartialSafe(edge.address(bundle), bundle.size),
        "'A' channel carries PutPartial type unsupported by manager" + extra
      )
    ...
    when(bundle.opcode === TLMessages.ArithmeticData) {
      monAssert(
        edge.manager.supportsArithmeticSafe(edge.address(bundle), bundle.size),
        "'A' channel carries Arithmetic type unsupported by manager" + extra
      )
    ...
    when(bundle.opcode === TLMessages.LogicalData) {
      monAssert(
        edge.manager.supportsLogicalSafe(edge.address(bundle), bundle.size),
        "'A' channel carries Logical type unsupported by manager" + extra
      )
    ...
    when(bundle.opcode === TLMessages.Hint) {
      monAssert(
        edge.manager.supportsHintSafe(edge.address(bundle), bundle.size),
        "'A' channel carries Hint type unsupported by manager" + extra
      )
  ...
  def legalizeFormatB(bundle: TLBundleB, edge: TLEdge) {
    ...
    monAssert(
      visible(edge.address(bundle), bundle.source, edge),
      "'B' channel carries an address illegal for the specified bank visibility"
    )
    ...
    val mask       = edge.full_mask(bundle)
    val legal_source =
      Mux1H(edge.client.find(bundle.source), edge.client.clients.map(c => c.sourceId.start.U)) === bundle.source

    when(bundle.opcode === TLMessages.Probe) {
      monAssert(
        edge.client.supportsProbe(bundle.source, bundle.size),
        "'B' channel carries Probe type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.Get) {
      monAssert(
        edge.client.supportsGet(bundle.source, bundle.size),
        "'B' channel carries Get type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.PutFullData) {
      monAssert(
        edge.client.supportsPutFull(bundle.source, bundle.size),
        "'B' channel carries PutFull type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.PutPartialData) {
      monAssert(
        edge.client.supportsPutPartial(bundle.source, bundle.size),
        "'B' channel carries PutPartial type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.ArithmeticData) {
      monAssert(
        edge.client.supportsArithmetic(bundle.source, bundle.size),
        "'B' channel carries Arithmetic type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.LogicalData) {
      monAssert(
        edge.client.supportsLogical(bundle.source, bundle.size),
        "'B' channel carries Logical type unsupported by client" + extra
      )
    ...
    when(bundle.opcode === TLMessages.Hint) {
      monAssert(
        edge.client.supportsHint(bundle.source, bundle.size),
        "'B' channel carries Hint type unsupported by client" + extra
      )
  ...
  def legalizeFormatC(bundle: TLBundleC, edge: TLEdge) {
    ...
    monAssert(
      visible(edge.address(bundle), bundle.source, edge),
      "'C' channel carries an address illegal for the specified bank visibility"
    )
    ...
    when(bundle.opcode === TLMessages.ProbeAckData) {
      ...
      monAssert(
        bundle.size >= log2Ceil(edge.manager.beatBytes).U,
        "'C' channel ProbeAckData smaller than a beat" + extra
      )
    ...
    when(bundle.opcode === TLMessages.Release) {
      monAssert(
        edge.manager.supportsAcquireBSafe(edge.address(bundle), bundle.size),
        "'C' channel carries Release type unsupported by manager" + extra
      )
      monAssert(
        edge.client.supportsProbe(edge.source(bundle), bundle.size),
        "'C' channel carries Release from a client which does not support Probe" + extra
      )
    ...
    when(bundle.opcode === TLMessages.ReleaseData) {
      monAssert(
        edge.manager.supportsAcquireBSafe(edge.address(bundle), bundle.size),
        "'C' channel carries ReleaseData type unsupported by manager" + extra
      )
      monAssert(
        edge.client.supportsProbe(edge.source(bundle), bundle.size),
        "'C' channel carries Release from a client which does not support Probe" + extra
      ...
      monAssert(
        bundle.size >= log2Ceil(edge.manager.beatBytes).U,
        "'C' channel ReleaseData smaller than a beat" + extra
      )
  ...
  def legalizeADSourceFormal(bundle: TLBundle, edge: TLEdge) {
    ...
    when(
      (my_resp_opcode === TLMessages.Get) || (my_resp_opcode === TLMessages.ArithmeticData) ||
    ...
    monAssert(
      IfThen(my_resp_pend, !my_a_first_beat),
    ...
    assume(
      IfThen(my_clr_resp_pend, (my_set_resp_pend || my_resp_pend)),
    ...
    assume(
      IfThen(my_d_first_beat, (my_a_first_beat || my_resp_pend)),
    ...
    assume(
      IfThen(my_d_first_beat, (bundle.d.bits.size === my_resp_size)),
      "If d_valid is 1, then d_size should be same as a_size of the corresponding request" +
        "message" + extra
    )
    assume(
      IfThen(my_d_first_beat, my_resp_opcode_legal),
  ...
  def legalizeADSourceOld(bundle: TLBundle, edge: TLEdge) {
    ...
    val limit    = PlusArg("tilelink_timeout", docstring = "Kill emulation after INT waiting TileLink cycles. Off if 0.")
  ...
  def legalizeADSource(bundle: TLBundle, edge: TLEdge) {
    val a_size_bus_size =
      edge.bundle.sizeBits + 1 //add one so that 0 is not mapped to anything (size 0 -> size 1 in map, size 0 in map means unset)
    ...
    a_opcode_lookup := ((inflight_opcodes) >> (bundle.d.bits.source << log_a_opcode_bus_size.U) & size_to_numfullbits(
      1.U << log_a_opcode_bus_size.U
    )) >> 1.U

    val a_size_lookup = WireInit(0.U((1 << log_a_size_bus_size).W))
    a_size_lookup.suggestName("a_size_lookup")
    a_size_lookup := ((inflight_sizes) >> (bundle.d.bits.source << log_a_size_bus_size.U) & size_to_numfullbits(
      1.U << log_a_size_bus_size.U
    )) >> 1.U

    val responseMap = VecInit(
      Seq(
    ...
    val responseMapSecondOption = VecInit(
      Seq(
        TLMessages.AccessAck,
        TLMessages.AccessAck,
        TLMessages.AccessAckData,
    ...
    when(bundle.d.valid && d_first && edge.isResponse(bundle.d.bits) && !d_release_ack) {
      assume(
        ((inflight)(bundle.d.bits.source)) || (bundle.a.valid && (bundle.a.bits.source === bundle.d.bits.source) && (bundle.a.bits.size === bundle.d.bits.size) && a_first),
        "'D' channel acknowledged for nothing inflight" + extra
      )
      assume(
        ((bundle.d.bits.opcode === responseMap(a_opcode_lookup)) || (bundle.d.bits.opcode === responseMapSecondOption(
          a_opcode_lookup
        )))
          || (bundle.a.valid && ((bundle.d.bits.opcode === responseMap(bundle.a.bits.opcode)) || (bundle.d.bits.opcode === responseMapSecondOption(
            bundle.a.bits.opcode
          )))),
        "'D' channel contains improper opcode response" + extra
      )
      assume(
        (bundle.d.bits.size === a_size_lookup) || (bundle.a.valid && (bundle.a.bits.size === bundle.d.bits.size)),
        "'D' channel contains improper response size" + extra
      )
    }
    when(
      bundle.d.valid && d_first && a_first && bundle.a.valid && (bundle.a.bits.source === bundle.d.bits.source) && !d_release_ack
    ) {
  ...
  def legalizeUnique(bundle: TLBundle, edge: TLEdge) {
    ...
    if (edge.client.anySupportProbe && edge.manager.anySupportAcquireB) {
      ...
      if (sinkBits > tooBig) {
        println(
          s"WARNING: TLMonitor instantiated on a bus with sink bits (${sinkBits}) > ${tooBig}; D=>E transaction flight will not be checked"
        )
      } else {
